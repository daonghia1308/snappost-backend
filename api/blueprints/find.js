...
module.exports = function findRecords(req, res) {
    ...
    let countQuery = Model.count().where(actionUtil.parseCriteria(req));
    countQuery.exec(function f(err, count) {
        ...
        Model.find(queryOptions.criteria, queryOptions.populates)
            ...
            .exec(function found(err, matchingRecords) {
                if (matchingRecords.length > MAX_RECORDS) {
                    return res.serverError({
                        code: 502,
                        message: 'Số lượng kết quả trả về quá lớn',
                    });
                }
                if (err) {
                ...
                if (req.apiInfo && req.apiInfo.downloadReport) {
                    switch (req.apiInfo.downloadReport) {
                        case 'codeReport':
                            return sails.helpers.report
                                .reportCode(matchingRecords)
                                .then(bin => {
                                    res.setHeader(
                                        'Content-Type',
                ...
                if (queryOptions.using === 'notice') {
                    ...
                    if (updateId.length) {
                        Model.update({ id: updateId }, { isRead: true }).exec(function f(
                            err,
                            record
                        ) {
                            if (err) {
